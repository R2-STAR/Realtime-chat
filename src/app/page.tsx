...
const Page = () => {  //makes the Page component wait until Lobby is fully loaded before rendering anything.
  return (
    <Suspense>
      <Lobby />
    </Suspense>
...
function Lobby() {
  ...
  const searchParams = useSearchParams()  //lets you read values from the URL query string
  ...
  const { mutate: createRoom } = useMutation({
    mutationFn: async () => {
      ...
      if(res.status === 2) {
        router.push(`/room/${res.data?.roomId}`)   //Redirects the user to the newly created room page.
      }
    },
  })
